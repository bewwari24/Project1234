    ...
    initialize: function(elem) {
      if (elem.checkbox !== null) {
        ...
        function _doChanged(values) {
          display.innerText = values.length == 0 ? placeholder : '+' + values.length + ' ' + trans('items');
        ...
        select.setOptions = function(json, value) {
          list = [];
          dropdown.innerHTML = '';
          listindex = 0;
          for (var key in json) {
            select.addItem(key, options[key], value.indexOf(key) > -1);
          }
        };
